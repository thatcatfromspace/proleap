...
export const Cards = ({ uid, Activity, setShowActivity, setShowCard }) => {
  ...
  const previousCard = (e) => {
    ...
    if (currentCard.sequence_no != 1) {
      let card = cards.find((element) => element.sequence_no == currentCard.sequence_no - 1);
      setCurrentCard(card);
  ...
  const nextCard = (e) => {
    ...
    if (!required) {
      ...
      for (let i = 0; i < currentCard.questions.length; i++) {
        console.log(currentCard.questions[i]);
        ...
        if (currentCard.type === "RADIO") {
          let obj = {
            ...
            answer: "",
            user: userId,
            option: currentCard.questions[i].answer[0].id,
          };
          console.log("RADIO BUTTON");
          console.log(obj);
          // TODO: remove activity and card
          axios
            .post(`http://${import.meta.env.VITE_API_URL}/apis/answer/`, obj)
            .then((res) => {
              console.log(res.data);
            });
        } else if (
          currentCard.type === "TEXT" ||
          currentCard.type === "SHORT_ANSWER" ||
          currentCard.type === "DATE"
        ) {
          let obj = {
            question: currentCard.questions[i].id,
            answer: currentCard.questions[i].answer,
            user: userId,
            option: "",
