...
def update_user_progress(sender, instance, created, **kwargs):
    try:
        ...
        user_cards_in_activity = UserCard.objects.filter(
            user=user, card__in=card_ids)
        completed_cards_count = user_cards_in_activity.filter(
            status=Status.COMPLETED).count()

        # Update the UserActivity instance
        user_activity, created = UserActivity.objects.get_or_create(
            user=user, activity=card.activity)
        ...
        logger.info(
            f"UserActivity updated: {
                user_activity.status}, {
                user_activity.completed_cards}")

        # Fetch all activities for the batch of the current activity
        activities_in_batch = Activity.objects.filter(
            batch=card.activity.batch)
        activity_ids = activities_in_batch.values_list('id', flat=True)
        user_activities_in_batch = UserActivity.objects.filter(
            user=user, activity__in=activity_ids)
        completed_activities_count = user_activities_in_batch.filter(
            status=Status.COMPLETED).count()

        # Update the UserBatch instance
        user_batch = UserBatch.objects.get(
            user=user, batch=card.activity.batch)
        ...
        logger.info(
            f"UserBatch updated: {
                user_batch.status}, {
                user_batch.completed_activities}")

    except UserCard.DoesNotExist:
        print(
            f"UserCard instance for user {
                user.id} and card {
                card.id} does not exist.")
    except UserActivity.DoesNotExist:
        print(
            f"UserActivity instance for user {
                user.id} and activity {
                card.activity.id} does not exist.")
    except UserBatch.DoesNotExist:
        print(
            f"UserBatch instance for user {
                user.id} and batch {
                card.activity.batch.id} does not exist.")
